...
Optional Environment Variables (with defaults):
  ...
  BUILD_FAIL_EXIT_CODE   Exit code to use when the PyTorch build fails (default: 125).
                         125 = let git bisect skip this commit and continue.
                         Set to 128 to abort the entire bisect on the first build
                         failure. Must be one of: 0, 1-124, 125, 126-127, 128.
...
# Exit code to use when the PyTorch build fails. Defaults to 125 so that git
# bisect skips the bad commit and continues. Set to 128 to abort the entire
# bisect on the first build failure instead.
BUILD_FAIL_EXIT_CODE=${BUILD_FAIL_EXIT_CODE:-125}

# Validate BUILD_FAIL_EXIT_CODE: must be a non-negative integer in [0, 128].
# Reject anything else so we never silently swallow a build failure as exit 0.
