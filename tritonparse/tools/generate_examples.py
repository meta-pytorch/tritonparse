...
# The inductor example is new, so its token is chosen rather than inherited.
# It still has to be a USER token in the same filename shape -- unified_parse
# rejects any basename without the dedicated_log_triton_trace_ prefix, so a
# free-form name would not be re-parseable by our own CLI.
INDUCTOR_TRACE_TOKEN = "inductor"
...
def _run_inductor() -> None:
    """torch.compile kernels: what most users' traces actually contain.

    Covers both shapes inductor generates -- a fused pointwise kernel
    (``triton_poi_*``) and persistent reductions (``triton_per_*``) -- so the
    example shows kernels nobody hand-wrote, whose Python source is inductor's
    generated ``output_code.py`` and whose compilations carry ``pt_info``
    frame attribution.

    Two inductor settings are not optional here:

    ``compile_threads = 1`` keeps codegen inline. Above 1, inductor forks
    compile workers after a ``pre_fork_setup()`` that initializes CUDA, which
    on cu130 makes ``cuInit`` return CUDA_ERROR_NOT_INITIALIZED in the child
    whenever ~/.triton/cache is cold. The same note is in
    tests/gpu/test_multiprocess_write_inductor.py.
    ...
    def reduction(t):
        return torch.softmax(t, dim=-1).sum()

    previous_threads = inductor_config.compile_threads
    inductor_config.compile_threads = 1
    try:
        x = torch.randn(1024, 1024, device="cuda")
        torch.compiler.reset()
        with inductor_config.patch(force_disable_caches=True):
            for fn in (pointwise, reduction):
                torch.compile(fn, dynamic=False, fullgraph=True)(x)
        torch.cuda.synchronize()
    finally:
        inductor_config.compile_threads = previous_threads
...
WORKLOADS: Dict[str, Workload] = {
    ...
    "inductor": Workload(
        name="inductor",
        description=(
            "torch.compile: a fused pointwise kernel plus persistent "
            "reductions, generated by inductor rather than hand-written."
        ),
        run=_run_inductor,
        init_kwargs={"enable_trace_launch": True, "enable_sass_dump": True},
        trace_token=INDUCTOR_TRACE_TOKEN,
        website_example=True,
        fixture_subdir="parsed_output_inductor",
    ),
}
