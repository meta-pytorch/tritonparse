...
| `enable_full_python_source` | `Optional[bool]` | `None` | Capture the whole Python source file instead of only the kernel's function definition. `True` forces full-file capture, `False` forces function-only capture, `None` defers to `TRITON_FULL_PYTHON_SOURCE`. Recommended for nested kernels, where the traced `@triton.jit` entry point is only a thin wrapper around the real kernel. |
...
- Clears the `enable_full_python_source` override, so `TRITON_FULL_PYTHON_SOURCE` is back in charge

> **Note**: `enable_sass_dump` is the exception — its override also carries the value of `TRITONPARSE_DUMP_SASS`, which is read at import time, so it is left in place rather than cleared.
...
| `split_inductor_compilations` | `bool` | `True` | Split output by frame_id, compile_id, attempt, and compiled_autograd_id. |
...
| `no_pre_init_attribution` | `bool` | `False` | Disable attribution of pre-distributed-init trace files to their owning rank. |
| `kernel_allowlist` | `Optional[str]` | `None` | Comma-separated `fnmatch` patterns. Keeps complete matching kernel and autotune event groups. |
...
| `enable_trace_launch_within_profiling` | `bool` | `False` | Forwarded to `init()` unchanged. See the `init()` parameter above. |
| `enable_more_tensor_information` | `bool` | `False` | Forwarded to `init()` unchanged. See the `init()` parameter above. |
...
| `enable_sass_dump` | `Optional[bool]` | `None` | Forwarded to `init()` unchanged. See the `init()` parameter above. |
| `enable_full_python_source` | `Optional[bool]` | `None` | Forwarded to `init()` unchanged. See the `init()` parameter above. |
| `tensor_storage_quota` | `Optional[int]` | `None` | Storage quota for tensor blobs. |
| `compression` | `Optional[str]` | `None` | Forwarded to `init()` unchanged. See the `init()` parameter above. |
| `tensor_save_skip_runs` | `Optional[int]` | `None` | Skip tensor blob saving for the first N kernel runs. |
| `tensor_save_max_runs` | `Optional[int]` | `None` | Save tensor blobs for at most N kernel runs after skipping. |
...
| `**parse_kwargs` | | | Additional arguments passed to `unified_parse()`, such as parse-time `kernel_allowlist`. These do not change trace capture inside the context. |
...
| `--trace` | Compare all kernels across two trace files |
| `--ai` | Run AI analysis on detected differences |
| `--ai-model` | LLM model for AI analysis (default: auto-select) |
| `--ai-provider` | LLM provider for AI analysis: `claude`, `codex`, or `muse` (default: `claude`) |

Note: the `muse` provider has no hard read-only enforcement — the analysis
prompt instructs read-only behavior, but this is not mechanically guaranteed
the way `claude`'s tool allowlist is.
...
| `--ai-provider` | LLM provider for AI fix: `claude`, `codex`, or `muse` (default: `claude`) |
