...
### Q: How do I parse only selected kernels from an existing trace?

**A:** Pass comma-separated `fnmatch` patterns to the parse command:

```bash
tritonparseoss parse ./logs/ --kernel-allowlist 'matmul*,*attention*'
```

The parser retains complete kernel-hash and autotune-session event groups, so
the resulting analyses are not truncated. `TRITONPARSE_KERNEL_ALLOWLIST` is a
separate trace-time setting and is not implicitly applied while parsing old
logs. Pre-parsed CLP output cannot be filtered; provide the original structured
trace instead.
...
### Q: My trace filenames now contain `_rank_none_pid_12345_host_devgpu001_` — is that new?

**A:** Yes. The schema is `_rank_*_pid_{PID}_host_{HOST}_`, where `*` is
either an integer rank or the literal `none` (for pre-`dist.init` or
single-GPU runs). The PID + host segments isolate each process to
prevent multi-process write corruption and disambiguate PIDs across
machines. The always-present `_rank_*_` token lets MAST downloads use
a single regex `LOG_PREFIX.*_rank_(N|none)_` to fetch rank N + the
matching pre-init no-rank file in one call — no prefix-match
overdownload needed. Older files without one or more of these suffixes
still parse locally, but legacy files lacking the `_rank_*_` token are
not pulled by MAST `--rank N` / `--rank none` (use `--all-ranks`). See
[Trace File Naming](02.-Usage-Guide.md#trace-file-naming).
...
compiled before `dist.init_process_group()` previously appeared only
under `rank_none`. They now show up under their owning rank because
TritonParse can correlate the no-rank trace file with the ranked one
via the shared `(host, pid)` identifier. To restore the old behavior
(separate `rank_none` bucket), pass `--no-pre-init-attribution`.
