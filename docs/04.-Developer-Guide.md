...
# Click "Default Example" in the browser, which loads
# ./public/dedicated_log_triton_trace_findhao__mapped.ndjson.gz
```

Regenerate that example after changing what the tracer captures:

```bash
cd .. && make regen-examples-install   # requires a CUDA GPU
```
