...
Two example traces ship with the site. Regenerate them with
`make regen-examples-install WORKLOAD=triton` and
`make regen-examples-install WORKLOAD=inductor`, respectively:

| Example | Contents |
|---------|----------|
| [Hand-written Triton](https://meta-pytorch.org/tritonparse/?json_url=https://meta-pytorch.org/tritonparse/dedicated_log_triton_trace_findhao__mapped.ndjson.gz) | An autotuned matmul and a fused elementwise kernel. Has launch data, so launch diffs, per-launch roofline and the autotune session views are all populated. |
| [Inductor](https://meta-pytorch.org/tritonparse/?json_url=https://meta-pytorch.org/tritonparse/dedicated_log_triton_trace_inductor__mapped.ndjson.gz) | Kernels `torch.compile` generated, across two compile frames. Their Python source is inductor's generated `output_code.py`, not a kernel anyone wrote by hand. |

The inductor example has no launch-level data. tritonparse's launch metadata is
attached by its JIT hook, and inductor launches through its own
`CachingAutotuner` launcher, which does not carry that payload — so launch
diffs and roofline are empty for it.
