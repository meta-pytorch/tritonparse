...
adapter = get_backend_registry().resolve_from_backend_name('your_adapter_name')
...
class YourAdapter(CompilationPipelineAdapter):
    adapter_name: str = "your_backend_name"
...
|-----------|-------------|
| `runtime_backend` | The target runtime backend. This field should match the `backend_name` defined in the target backend's Triton compiler implementation. For the current built-in backends, NVIDIA corresponds to `"cuda"`, and Ascend corresponds to `"cann"`. |
| `adapter_name` | The combined identifier of the target runtime backend and compilation chain type. Since TritonParse currently only supports the Triton compilation chain, the naming convention is `"{runtime_backend}_triton"`. For example, `"cuda"` → `"cuda_triton"`, `"cann"` → `"cann_triton"`. |
| `pytorch_module` | The PyTorch device module name in the target runtime environment, i.e., the part after `torch.` in code. For example, NVIDIA uses `torch.cuda`, so this field is `"cuda"`; Ascend uses `torch.npu`, so this field is `"npu"`. |
...
TritonParse includes two built-in Analyzers:
...
NVIDIA's CUBIN → SASS derivation logic is located in the `extract` function in [disasm.py](../tritonparse/tools/disasm.py), which calls `nvdisasm` to disassemble CUBIN into SASS.
